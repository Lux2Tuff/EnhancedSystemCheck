    ...
    if ($Vars -contains "-dev") {
        ...
        foreach ($module in $modules) {
            try {
                ...
                Write-Log "Failed to load module $module from $url : $($_.ToString())" "ERROR"
            }
        }
    }
}
...
function Invoke-CheatDetection {
    ...
    foreach ($process in $processes) {
        try {
            ...
            Write-Log "Error scanning modules for process $($process.Name): $($_.ToString())" "ERROR"
        }
    }
}
...
function Invoke-MalwareDetection {
    ...
    foreach ($path in $pathsToScan) {
        try {
            ...
            Write-Log "Error scanning path $path: $($_.ToString())" "ERROR"
        }
    }
...
function Get-PCIeSlotData {
    ...
    foreach ($device in $pnpDevices) {
        try {
            ...
            Write-Log "Error processing PCIe device: $($_.ToString())" "ERROR"
        }
    }
}
...
"@
    try {
        ...
        Write-Log "Failed to generate summary markdown: $($_.ToString())" "ERROR"
    }
}
...
function Main {
    ...
    try {
        ...
        Write-Log "Failed to set execution policy: $($_.ToString())" "ERROR"
        Exit 1
    }
    ...
    try {
        ...
        Write-Log "Failed to copy summary to clipboard: $($_.ToString())" "ERROR"
    }
